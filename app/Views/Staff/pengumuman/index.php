...
<div class="page-header">
    <div>
        <h4>Pengumuman</h4>
        <div class="text-muted small">Kelola pengumuman desa.</div>
    </div>
    <div class="page-header-actions">
        <div class="page-header-icon">
            <i class="bi bi-megaphone"></i>
        </div>
        <a href="<?= base_url('/staff/pengumuman/tambah') ?>" class="page-header-icon page-header-icon-add" title="Tambah Pengumuman">
...
$(document).ready(function() {
    ...
    function loadPengumuman(page, search = '', itemsPerPage = limit) {
        if (isLoading) return;
        
        isLoading = true;
        $('#loadingIndicator').show();
        $('#pengumumanContainer').empty();
        ...
        $.ajax({
            url: '<?= base_url('/staff/pengumuman/api') ?>',
            type: 'GET',
            data: {
                page: page,
                limit: itemsPerPage,
            ...
            success: function(response) {
                if (response.data && response.data.length > 0) {
                    ...
                    response.data.forEach(function(item) {
                        ...
                        const thumbnail = item.thumbnail ? '<?= base_url() ?>/' + item.thumbnail : '';
                        
                        html += '<div class="col-md-6 col-lg-4 col-xl-3">' +
                            '<div class="card border-0 shadow-sm h-100 gallery-card">';
                        
                        if (thumbnail) {
                            html += '<img src="' + thumbnail + '" class="card-img-top gallery-card-img" alt="' + item.judul + '" loading="lazy">';
                        } else {
                            html += '<div class="bg-light d-flex align-items-center justify-content-center" style="height: 200px;">' +
                        ...
                        html += '<div class="card-body">' +
                            ...
                            '<a href="<?= base_url('/staff/pengumuman/') ?>' + item.id + '/edit" class="btn btn-sm btn-outline-primary flex-fill"><i class="bi bi-pencil"></i> Edit</a>' +
                            '<a href="<?= base_url('/staff/pengumuman/') ?>' + item.id + '/hapus" class="btn btn-sm btn-outline-danger flex-fill" onclick="return confirm(\'Hapus pengumuman ini?\')"><i class="bi bi-trash"></i> Hapus</a>' +
                            '</div>' +
                            '</div>' +
                            '</div>' +
                            '</div>';
                    });
                    $('#pengumumanContainer').html(html);
    ...
    function goToPage(page) {
        ...
        loadPengumuman(page, currentSearch, limit);
    ...
    $('#lengthSelect').on('change', function() {
        ...
        loadPengumuman(1, currentSearch, limit);
    });
