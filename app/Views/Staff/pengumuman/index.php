...
.pengumuman-img-wrapper img {
    width: 100%;
    height: 100%;
    object-fit: cover;
    transition: transform 0.6s cubic-bezier(0.4, 0, 0.2, 1);
}
.pengumuman-card:hover .pengumuman-img-wrapper img {
    transform: scale(1.05);
}
.pengumuman-date-pill {
    position: absolute;
    top: 12px;
    left: 12px;
...
.pengumuman-action-edit {
    color: #10b981;
    background: #ecfdf5;
}
.pengumuman-action-edit:hover {
    background: #d1fae5;
    color: #059669;
}
.pengumuman-action-delete {
    color: #ef4444;
    background: #fef2f2;
}
.pengumuman-action-delete:hover {
    background: #fee2e2;
    color: #dc2626;
}
...
<div class="mb-4 mt-2 d-flex flex-column flex-md-row justify-content-between align-items-md-end gap-3">
    <div>
        ...
        <h2 class="fw-bold text-dark mb-2" style="font-size: 2.2rem; letter-spacing: -0.5px;">
            Pengumuman <span style="color: #15803d;">Desa</span>
        </h2>
        <p class="text-muted fs-6 mb-0" style="max-width: 600px;">Kelola pengumuman desa.</p>
    </div>
    <div class="d-flex flex-wrap align-items-center gap-2">
        <div class="d-flex align-items-center bg-white border px-3 search-bar-container" style="width: 260px; height: 38px; border-radius: 0.5rem;">
            ...
            <input type="text" id="searchInput" class="form-control border-0 shadow-none bg-transparent px-2 w-100" placeholder="Cari pengumuman..." style="height: 100%;">
...
<div id="pengumumanContainer" class="row g-3">
    <!-- Pengumuman akan di-load via AJAX -->
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadPengumuman(page, search = '') {
        ...
        fetch('<?= base_url('/staff/pengumuman/api') ?>', {
            method: 'POST',
            body: formData,
            ...
            if (response.success && response.data && response.data.length > 0) {
                ...
                response.data.forEach(function (item) {
                    const imgHtml = item.thumbnail
                        ? `<img src="${escapeHtml(item.thumbnail)}" alt="${escapeHtml(item.judul)}" loading="lazy" onerror="this.onerror=null;this.parentElement.innerHTML='<div class=\\'pengumuman-placeholder\\'><i class=\\'bi bi-megaphone\\'></i></div>'">`
                    ...
                    <div class="col-md-6 col-lg-4 col-xl-3">
                        <div class="pengumuman-card h-100">
                            ...
                            <div class="pengumuman-card-body">
                                ...
                                <p class="pengumuman-excerpt">${escapeHtml(item.isi)}</p>
                                <div class="pengumuman-footer">
                                    <a href="<?= base_url('/staff/pengumuman/') ?>${item.id}/edit" class="pengumuman-action pengumuman-action-edit stretched-link">
                                        <i class="bi bi-pencil-square"></i> Edit
                                    </a>
                                    <button type="button"
                                        class="pengumuman-action pengumuman-action-delete btn-hapus-pengumuman"
                                        data-id="${item.id}"
    ...
    function attachDeleteEvents() {
        document.querySelectorAll('.btn-hapus-pengumuman').forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                showConfirm(`Hapus pengumuman "${judul}"? Tindakan ini tidak dapat dibatalkan.`, 'Hapus Pengumuman', 'Ya, Hapus').then(confirmed => {
                    if (!confirmed) return;
                    window.location.href = `<?= base_url('/staff/pengumuman/') ?>${id}/hapus`;
                });
            });
        });
    }
    ...
    function goToPage(page) {
        ...
        loadPengumuman(page, currentSearch);
