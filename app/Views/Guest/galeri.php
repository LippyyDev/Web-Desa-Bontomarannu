...
<style>
/* Search box — same style as Berita */
.galeri-search-box {
    background: #ffffff;
    border-radius: 50px;
    padding: 6px 10px;
...
.galeri-search-box input::placeholder { color: #94a3b8; }
.galeri-search-box .btn-search {
    border-radius: 50px;
    padding: 8px 24px;
    background: #2E7D32;
    color: #fff;
...
.galeri-search-box .btn-search:hover { background: #1b5e20; }
.galeri-search-box .btn-reset {
    background: #f1f5f9;
    color: #64748b;
    border-radius: 50%;
    width: 36px;
...
.galeri-search-box .btn-reset:hover { background: #e2e8f0; color: #ef4444; }

/* Skeleton */
.skeleton-galeri-card {
    border-radius: 24px;
    background: #fff;
    border: 1px solid #edf2f7;
    overflow: hidden;
...
<section class="profil-desa-section pt-0" style="margin-top: -2.5rem;">
    <div class="container pb-5">
        ...
        <!-- Search Box -->
        <div class="galeri-search-box reveal-up active delay-100">
        ...
        <!-- Data Container -->
        <div id="galeriContainer" class="row g-4">
            <!-- Loaded via AJAX -->
        </div>
        ...
        <div id="loadingIndicator" class="row g-4">
            ...
            <div class="col-md-6 col-lg-4">
                <div class="skeleton-galeri-card">
        ...
        <div id="emptyMessage" class="text-center text-muted py-5" style="display:none;">
            <i class="bi bi-images fs-1 d-block mb-3" style="color:#cbd5e1;"></i>
            <p class="mb-0 fs-5">Belum ada album galeri.</p>
        </div>
        ...
        <div id="noResultsMessage" class="text-center text-muted py-5" style="display:none;">
            ...
            <p class="mb-0 fs-5">Tidak ada album yang cocok dengan pencarian.</p>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadData(page, search) {
        ...
        .then(response => {
            ...
            response.data.forEach(function (item) {
                const imgHtml = item.cover_url
                    ? `<img src="${escapeHtml(item.cover_url)}"
                            alt="${escapeHtml(item.nama_album)}"
                            loading="lazy"
                            onerror="this.onerror=null;this.outerHTML='<div class=\\'galeri-placeholder\\'><i class=\\'bi bi-images\\'></i></div>'">`
                ...
                <div class="col-md-6 col-lg-4 reveal-up">
                    <div class="galeri-card">
                        ...
                        <div class="galeri-card-body">
                            <h5 class="galeri-title">${escapeHtml(item.nama_album)}</h5>
                            <div class="galeri-excerpt">${escapeHtml(truncate(item.deskripsi, 100))}</div>
                            <a href="<?= base_url('/galeri/') ?>${item.id}" class="galeri-read-more stretched-link mt-2">
                                Lihat Album <span class="arrow">→</span>
                            </a>
                        </div>
                    </div>
                </div>`;
            ...
            galeriContainer.querySelectorAll('.reveal-up').forEach(el => revealObserver.observe(el));
        })
        .catch(() => {
            loadingIndicator.style.display = 'none';
            noResultsMessage.style.display = 'block';
