...
class ProfileController extends ProtectedController
{
    ...
    public function update()
    {
        ...
        if ($db->table('users')->where('username', $newUsername)->where('id !=', $uid)->get()->getRow()) {
            return redirect()->to('/staff/profil')->with('error', 'Username "' . esc($newUsername) . '" sudah digunakan oleh akun lain.');
        }

        // Cek duplikat email (kecuali akun sendiri)
        if ($db->table('users')->where('email', $newEmail)->where('id !=', $uid)->get()->getRow()) {
            return redirect()->to('/staff/profil')->with('error', 'Email "' . esc($newEmail) . '" sudah digunakan oleh akun lain.');
        }
        ...
        if ($file && $file->isValid()) {
            ...
            if (!in_array($extension, $allowedExtensions)) {
                return redirect()->to('/staff/profil')->with('error', 'Format foto profil hanya boleh JPG, JPEG, atau PNG.');
            }

            // Validasi Ukuran (Max 1 MB)
            if ($file->getSize() > 1048576) {
                return redirect()->to('/staff/profil')->with('error', 'Ukuran foto profil maksimal 1 MB.');
            }
